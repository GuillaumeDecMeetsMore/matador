...
| `PartialSendError`                | One or more subscriber publishes failed during `send()`. Contains an `errors` array with per-subscriber details (`subscriberName`, `queue`, `error`). |
| `TransportSendError`              | Failed to send a single message through the transport. Wrapped inside `PartialSendError.errors[n].error`.                                             |
